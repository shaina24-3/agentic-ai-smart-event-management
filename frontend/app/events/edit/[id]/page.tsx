  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-8 md:px-6">
      <div className="mx-auto max-w-3xl">
        <button onClick={() => router.push("/events")} className="mb-5 text-sm font-medium text-blue-600">
          ← Back to My Events
        </button>
        <h1 className="text-3xl font-bold">Edit Event</h1>
        <form onSubmit={save} className="mt-8 space-y-5 rounded-2xl bg-white p-8 shadow-lg">
          {(["title", "description", "date", "time", "venue", "capacity"] as const).map((k) => (
            <div key={k}>
              <label className="mb-2 block text-sm font-medium">
                {k === "title" ? "Event Title" : k === "description" ? "Description" : k === "date" ? "Event Date" : k === "time" ? "Event Time" : k === "venue" ? "Venue" : "Capacity"}
              </label>
              {k === "description" ? (
                <textarea rows={4} value={form[k]} onChange={(e) => update(k, e.target.value)} className="w-full rounded-lg border px-4 py-3" />
              ) : (
                <input
                  type={k === "date" ? "date" : k === "time" ? "time" : k === "capacity" ? "number" : "text"}
                  min={k === "capacity" ? "1" : undefined}
                  value={form[k]}
          ...
          <div className="flex gap-4">
            <button type="button" onClick={() => router.push("/events")} className="w-1/2 rounded-lg border py-3 font-semibold">
              Cancel
            </button>
            <button className="w-1/2 rounded-lg bg-blue-600 py-3 font-semibold text-white">Update Event</button>
          </div>
        </form>
      </div>
    </main>
